...
class Swaps {
  ...
  private approveContract = async (privateKey: any, tokenAddress: string) => {
    const account = new ethers.Wallet(privateKey, this._provider).connect(
      this._provider
    );
  ...
  public swapExactTokensForETHSupportingFeeOnTransferTokens = async (
    ...
    try {
      ...
      const transaction =
        await swapContract.swapExactTokensForETHSupportingFeeOnTransferTokens(
          toHex(amountIn),
          toHex(amountOutMin),
          path,
          wallet.ADDRESS,
  ...
  public approve = async (
    wallet: { ADDRESS: string; PRIVATE_KEY: any },
    tokenAddress: string
  ) => {
    try {
      const MAX_INT =
      ...
      const nonce = await HelpersWrapper.getWalletNonce(wallet.ADDRESS);
      const contract = await this.approveContract(
        wallet.PRIVATE_KEY,
        tokenAddress
      );
